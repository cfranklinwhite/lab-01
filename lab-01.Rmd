...
```{r Q1}
#returns the number of rows in the visitors table
...
```{r Ex_f}
visitors %>% 
  filter(between(n_2022, 50000, 100000)) %>%
  count()
```
...
visitors %>% 
  summarise(mean_2021 = mean(n_2021), 
            median_2021 = median(n_2021)
            )
...
"Not Available" means that there are some incomplete entries in the data set (NA values), because we have not removed these entries the mean and median functions cannot give us answers.
...
It means that the data is not normally distributed and that there are some outliers with very large numbers of visitors pushing the mean up.
...
free_attractions <- visitors_with_nations %>%
